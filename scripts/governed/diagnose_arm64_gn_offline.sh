...
printf 'arm64-gn-diagnostic without-script=%s without-script-option-first=%s with-script=%s with-script-option-first=%s total-bytes=%s\n' \
  "$without_script_status" "$without_script_option_first_status" "$with_script_status" "$with_script_option_first_status" "$total"
